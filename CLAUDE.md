...
- **内容**：64 篇回响（`echo-*.html`），完整清单见 README《内容清单》。
...
- **不再写「思维工具」类文章**（站主 07·13 定）：即以某个认知偏误 / 效应 / 定律为主角、通篇科普该术语的写法（如已上线的 `echo-sunkcost` 沉没成本、`echo-survivor` 幸存者偏差——这两篇保留，但**不再新增同类**）。
  - 分寸：借历史故事或人物访谈讲道理是本站一贯做法，可以写；越界的是**把术语当主角**。
- **新选题动笔前先报站主定夺**，等点头再写。
- 选题回到站内既有几条线：通晓古今 / 日常的微光 / 身体知道 / 持久 vs 短暂 / 地理与历史感 / 个人记述 / 对话（「对话」类最少，值得多写；人物一律匿名化，不点名、不挂靠真实机构）。
